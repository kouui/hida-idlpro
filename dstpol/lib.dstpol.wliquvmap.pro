...
;********************************************************************
PRO wliquvmap, paths, dinfo, cal, prof_step=prof_step, selectwl=selectwl, pmax0=pmax0, bin=bin, savfolder=savfolder
...
iprof = transpose(reform(rebin(sa[nx/2-3:nx/2+3,*,0,nstep/2],1,ny,1,1)))
endelse
...
backg = uintarr(nx3*nsp+dd*(nsp+1),ny3+dd*2+(ny3+dd))
for j0=w1,w2 do begin  ; loop over wavelength
	...
	;window,wid,xs=nx3*nsp+dd*(nsp+1),ys=ny3+dd*2+(ny3+dd)
    tvscl,backg
	dispwliquvr,smap3,bin=bin,pmax=pmax,wid=wid,/wexist,yoffset=ny3+dd*2,coms=['I','Q','U','V']
