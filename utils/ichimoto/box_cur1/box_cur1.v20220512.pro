...
pro box_cur1, x0, y0, nx, ny, data=data, line=line, color=color
;+
; NAME:
;	BOX_CUR1
;
...
;
; KEYWORD PARAMETERS:
;	data  -- if set, return data coordinate
;
; OUTPUTS:
...
;	k.i., '03/10/28	  data, color, line keywords
;-

device, get_graphics = old, set_graphics = 6  ;Set xor
if not keyword_set(color) then col = !d.n_colors -1 else col=color
if not keyword_set(line) then line=0

if keyword_set(data) then begin
	cursor,x0,y0,1,/data	; push and set 1 corner
	x=x0 &	y=y0
	px = [x0, x, x, x0, x0] ;X points
	py = [y0, y0, y, y, y0] ;Y values
	while !err eq 1 do begin
		cursor, x, y, 2, /data 	;Wait for a button
		plots,px, py, col=col, /data, thick=1, lines=line   ;Draw the box
		px = [x0, x, x, x0, x0] ;X points
		py = [y0, y0, y, y, y0] ;Y values
		plots,px, py, col=col, /data, thick=1, lines=line   ;Draw the box
	endwhile
	...
	plots,px, py, col=col, /dev, thick=1, lines=line  ;Draw the box
	px = [x0, x, x, x0, x0] ;X points
	py = [y0, y0, y, y, y0] ;Y values
	plots,px, py, col=col, /dev, thick=1, lines=line  ;Draw the box
