...
root_dir = "/tmp_mnt/home/kouui/idlpro/works/cloud_model/cloudmodel"

if not isvalid(bg) then begin
    case 1 of
        max(abs(wl)) le 2.:aaa=double(rd_tfile(root_dir + '/idl_data/ref_Ha_filter_100.dat',2))
        max(abs(wl)) le 4.:aaa=double(rd_tfile(root_dir + '/idl_data/ref_Ha_filter_200.dat',2))
    endcase
...
;;  average of initial value of parameters over sub-region weighted by Sfin
