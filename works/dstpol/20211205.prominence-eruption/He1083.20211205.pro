...
;---------------------------------------------------------------------------
dir0 = '/mnt/HDD3TBn50/DST/sp/20211205/spec/HeI_10830/'		; root directory of raw dataset
caldir = 'calib/'					; holder for clib. data
setdir = 'scan/' 					;    "   for obs data
jf = 0 & ipos = 90 & pmax = 0.05			; spectrum #, pos on slit, max for disp
workdir = '/nwork/kouui/data-lvl1/dstpol/20211205.priminence-eruption/spec/'	; output directory in /nwork
outdir=setdir
wl0 = 10830. &	wl_range = 15. 				; central wavelength & range [A] 
wl_order = 0						; 0: top is red, 1: top is blue
darkfiles = findfile(dir0+caldir+'dark_50_*.fits') ; dark
flatfiles = findfile(dir0+caldir+'flat*.fits') ; flat
flatdark  = findfile(dir0+caldir+'dark_15_*.fits') ; dark for flat when expo. is deferent 
hairline = findfile(dir0+caldir+'hair*.fits')		; heirline data
polarizer = findfile(dir0+caldir+'pol*.fits')		; polarizer data
files = findfile(dir0+setdir+'pr3_*.fits')			; obs. data
avdfsav = workdir+'cal/avdforig_He.sav'			; averaged dark & flat
calparsav = workdir+'cal/calparam_He.sav'		; cp structure savefile
drkfltsav = workdir+'cal/drkflt_He.sav'			; drk[nx,ny] & fltl,fltr[nxp,ny,nn]
wlsav = workdir+'cal/wl_He.sav'				; wavelength array file
nstep = 10						; # of step for 1 scan
th_offset = 41.2					; WP offset in deg.
telpos='EAST'						; telescope position
scan_head_hhmmss = [14,6,12] ; hhmmss of the head of a scan sequence
use_scan_head = 1
;-----------------------------------------------------------------------------------
...
mm = get_dstmm(wl0,telpos,dst,sc=sc)
rmm = invert(mm)
;rmm[0,2]=-rmm[0,2]
s3 = s[*,*,0:3]
for j=0,3 do begin
