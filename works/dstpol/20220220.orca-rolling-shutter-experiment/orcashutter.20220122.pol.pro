...
date = '20220122'
spec = 'K_I_6697'
root = '/mnt/HDD3TBn51/DST/sp/'+date+'/spec/'+spec
folder = root+'/calib'
print, 'processing folder : ', folder
;goto, debug

fdatas = findfile(folder+'/pol*.fits')
...
window, 0, xs=nx/wfac, ys=ny/wfac
tvscl, rebin(data[*,*,0],nx/wfac,ny/wfac,1)
...
for i=0,n_elements(ys)-1 do begin
    ...
    plot, profl, xr=[0,30], xtitle='rotation step', ytitle='counting', charsize=charsize
endfor
...
plot, ys[0:iymax],phs[0:iymax], xtitle='y-pixel', ytitle='phase [rad]', yr=minmax(phs[9:iymax]), charsize=charsize

print, 'poly1d fitting the phase ...'
y0 = (iymax+1)/2
xx = ys[y0:iymax] & yy = phs[y0:iymax] 
;print, 'amp1=', amps[y0], '  ph1=', phs[y0]
coef = poly_fit(xx,yy,1)
;;coefs[*,i] = coef
print, 'f = y * ', coef[1],' + ', coef[0]
dt =  coef[1] * (1024/fix(bin)) / (2*!pi) /  rotp
print, 'time for single frame : ', dt ,' [s]'
oplot, xx, xx*coef[1]+coef[0], color='9900ff'x, line=2
...
fname = savedir+'/bin'+bin+'.exp'+exp+'ms.'+date+'.'+spec+'.pol.sav'
...
for i=0,n_elements(exparr)-1 do begin
    ...
    if count eq 0 then begin 
        plot, prf, psym=2, xr=[0,50], charsize=charsize;, psym=psym, symsize=0.1
        oplot, fit1, line=0
    endif else begin
        plot, prf, psym=2, xr=[0,50], charsize=charsize
        oplot, fit1, line=0
    endelse
...
for i=0,n_elements(exparr)-1 do begin
    ...
    print, 'amp1=', amps[y0], '  ph1=', phs[y0]
    if exp eq '50' then begin
    coef = poly_fit(xx[y0:iymax],yy[y0:iymax],1)
    ;;coefs[*,i] = coef
    print, 'f = y * ', coef[1],' + ', coef[0]
    dt =  coef[1] * (1024/fix(bin)) / (2*!pi) / rotp
    print, 'time for single frame : ', dt ,' [s]'
    ...
    if count eq 0 then begin 
        plot, xx, yy, yr=yr, color=colors[i], charsize=charsize
    endif else begin
