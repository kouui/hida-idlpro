...
x = (0.5 * (exps - 3.32) * 0.001) * 360. / rots
;y = tofs
;yf = x + 23.8

window, 10, xs=1000, ys=500
...
for i=0,1 do begin
if i eq 0 then plot, x[*,i], tofus[*,i], psym=2, symsize=2, color=_colors[i], charsize=2, yr=[25,35], xtitle= " 0.5 * (exposure[sec]-3.32*0.001) * 360. / rotp[sec],[deg] ", ytitle="th_offset,  [deg]" $
else oplot, x[*,i], tofus[*,i], psym=2, symsize=2, color=_colors[i]
endfor

bias = [24.3]
for i=0,1 do oplot, x[*,i], x[*,i]+bias[i],color=_colors[i]
